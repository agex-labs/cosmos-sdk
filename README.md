...
1. Open a PR against the current default branch (i.e. `agex-fork-v0.50.3`).
...
4. (In `agex-labs/agex`) `go mod edit -replace github.com/cosmos/cosmos-sdk=github.com/agex-labs/cosmos-sdk@$COMMIT_HASH`
5. (In `agex-labs/agex`) `go mod tidy`
6. (In `agex-labs/agex`) update package references in `mocks/Makefile`. See [here](https://github.com/agex-labs/agex/pull/848) for an example.
7. Open a PR in `agex-labs/agex` to bump the version of the fork.
...
2. Create a new branch off the desired CosmosSDK commit using tags. `git checkout -b agex-fork-$VERSION <CosmosSDK repo's tag name>`. The new branch should be named something like `agex-fork-$VERSION` where `$VERSION` is the version of CosmosSDK being forked (should match the CosmosSDK repo's tag name). i.e. `agex-fork-v0.50.3`.
3. Push the new branch (i.e `agex-fork-v0.50.3`).
4. Off of this new branch, create a new branch. (i.e `totoro/agexCommits`)
5. Cherry-pick each agex-created commit from the current default branch, in order, on to the new `agex-fork-$VERSION` branch (note: you may want to consider creating multiple PRs for this process if there are difficulties or merge conflicts). For example, `git cherry-pick <commit hash>`. You can verify the first commit by seeing the most recent commit sha for the `$VERSION` (i.e `v0.50.3`) tag on the `cosmos/cosmos-sdk` repo, and taking the next commit from that sha.
6. The agex fork uses an optimized version of `runTx` for handling ABCI CheckTx messages. Ensure that the logic within `runCheckTxConcurrently` stays up to date with the `runTx` counterpart by diffing [baseapp.go#runTx](https://github.com/agex-labs/cosmos-sdk/blob/6477dcf5693422fef693432e29bd979709aa995d/baseapp/baseapp.go#L754) current version against the new version copying over relevant changes to [baseapp.go#runCheckTxConcurrently](https://github.com/agex-labs/cosmos-sdk/blob/6477dcf5693422fef693432e29bd979709aa995d/baseapp/baseapp.go#L619).
7. Open a PR to merge the second branch (`totoro/agexCommits`) into the first (`agex-fork-v0.50.3`). Get approval, and merge. *DO NOT SQUASH AND MERGE. PLEASE REBASE AND MERGE*
8. Update `agex-labs/agex` by following the steps in "Making Changes to the fork" above.
9. Set `agex-fork-$VERSION` as the [default branch](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/managing-branches-in-your-repository/changing-the-default-branch) in this repository.
